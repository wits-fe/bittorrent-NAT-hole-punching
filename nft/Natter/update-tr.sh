...
# nft
nft add chain inet fw4 tr_dstnat
nft flush chain inet fw4 tr_dstnat
if ! nft list chain inet fw4 dstnat | grep -q 'jump tr_dstnat' > /dev/null; then
  nft add rule inet fw4 dstnat jump tr_dstnat
fi
nft add rule inet fw4 tr_dstnat iifname $interface $protocol dport $private_port counter dnat ip to $host:$port
...
if [ $forward_ipv6 -eq 1 ]; then
  nft add chain inet fw4 tr_forward_wan
  nft flush chain inet fw4 tr_forward_wan
  if ! nft list chain inet fw4 forward_wan | grep -q 'jump tr_forward_wan' > /dev/null; then
    nft insert rule inet fw4 forward_wan jump tr_forward_wan
  fi
  nft add rule inet fw4 tr_forward_wan iifname $interface meta nfproto ipv6 tcp dport $port counter accept
  nft add rule inet fw4 tr_forward_wan iifname $interface meta nfproto ipv6 udp dport $port counter accept
